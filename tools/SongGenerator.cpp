// Song content generator - a synth + Standard MIDI File writer for producing a chart's .wav/.mid
// pairs from code: square/triangle/bell/bass tones for melodic clips, noise+sine drum hits for
// percussion, mixed and written as 16-bit mono PCM WAV, plus a matching note chart using
// c_LaneMidiPitches (src/LaneConfig.h - keep in sync). Deterministic (seeded LCG noise).
//
// Not in CMakeLists.txt (no engine dependency) - compile and run directly:
//   g++ -O2 -std=c++20 -o song_gen.exe tools/SongGenerator.cpp
//   ./song_gen.exe        (from the repo root)
//
// Everything above main() is reusable (WAV/MIDI writers, DSP helpers, the Mixer, the voice
// renderers). main() generates every code-authored song, each in its own Generate*Song() function
// with its own dir/bpm/note tables. To author a new song, add a Generate*Song() and call it from
// main() - never overwrite main(), which once silently lost a song's note tables.
//
// Mix/DSP: GenerateMelodicClip/GenerateDrumClip take a targetPeak that normalizes one clip to a
// modest peak (~0.3-0.6), not a hot master - a chart's sections regularly stack several clips at
// once, and normalizing each toward 1.0 in isolation is what made the "AI Slop" songs clip.
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
//
// Four parallel feedback combs (each with a damped low-pass in its feedback path, so the tail
// darkens as it decays) summed and run through two series allpasses to diffuse it into a smooth
// wash. The biggest lever for not sounding like a dry synth demo.
struct CombFilter
{
...
// Runs dry through the reverb twice and keeps only the second pass, so the tail is already at its
// loop steady-state when playback starts. The output is exactly dry.size() samples (no appended
// tail), since ChartClip's timing math assumes a clip's audio length matches its authored beat length.
...
// Renders a melodic note of durationSeconds plus a short release tail so back-to-back staccato
// notes don't click. velocity is a linear gain multiplier (1.0 = full).
std::vector<double> RenderMelodicNote(double freqHz, double durationSeconds, Timbre timbre, double velocity = 1.0)
{
    ...
    // A low sustain plateau for square/triangle - a plateau near full level reads as a constant
    // wall of buzz with almost no dynamic shape. Bell keeps its own decaying sustain curve below.
...
// Nudges each event's timing/velocity by a small deterministic amount so a programmed pattern
// doesn't read as typed-in. Applied only to the audio event lists, never to the judged note chart,
// which must stay quantized to what ChartMidi loads.
...
// Renders a drum clip's audio from audioHits and, unless chartHits is empty, a separate note chart
// from chartHits - the two lists needn't agree on density (audio is what sounds good, the chart is
// what the player judges). See this file's top comment for targetPeak gain staging.
...
// audioEvents drives the WAV arrangement (as dense as sounds good); chartEvents drives the judged
// note chart, authored independently and typically much sparser. The runtime picks a clip's
// pattern by index and never inspects the audio, so the two needn't match density - a full
// arrangement over an easy chart is the intended use.
...
//
// The chart-note timings below are dumped from this song's committed .mid files, reproduced
// exactly so the judged difficulty its hits_required values were tuned against doesn't change.
// Only the audio arrangement and DSP/mix are authored here.
// ================================================================
...
// ================================================================
// Quiet Hours - a deliberately calm song: slower tempo, no Square leads, soft brushed drums, long
// breathing notes with rests between phrases, heavy reverb. A minor / C major, diatonic, mostly
// stepwise motion. Matches Content/Quiet Hours/Quiet Hours.chart's section layout.
// ================================================================
