...
- Use Allman braces (curly braces on a separate line directly below the control statement or function declaration, indented to the same level)
- All classes and function should have a header comment that describes what they represent and what they do.  For functions, it should start with a simple, one-sentence explanation of what it does, followed by a line for each parameter which explains what it represents (and it's units, where applicable).  For non-void functions, there should also be a line that describes what the return values represents (and it's units where applicable).  Comments should be descriptive, but as brief as possible.  A comment describes *what* something is and *what* it does — not *why* it exists, *why* it is called, or *why* it is designed a particular way; that reasoning belongs with the code that uses it, not the definition.  Do not restate what the language, ordinary C++ conventions, or a self-descriptive name and signature already make obvious (e.g. "the fields are private", "read through the getters below", "returns the clip at the given index").  Comments should not include any sort of history (i.e. "This function used to work like X...").  Complicated but important concepts that a caller genuinely needs go in the class header rather than scattered through function comments, and even there stay to what the concept *is*.  
- In-line comments (not function or class headers) should ideally by less than 2 lines long, but can occasionally be longer if it's important.
- Member variables do not need comments if they are obvious
- If a member variable or function does need a comment, it should go above the variable/function declaration.  If there is a code directly above it, there should be an empty line above the start of the comment 
- Whenever possible, do *not* include a non-library header file from another header file, use forward declarations instead.  Forward declarations do not need comments explaining their purpose, but wherever we include a header from another header, add a one-line comment explaining why we need to do that.
